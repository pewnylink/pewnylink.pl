# app/scrapers/scraper_engine.py
import re
import json
import httpx
from bs4 import BeautifulSoup
from typing import Dict, Any, List, Optional
from urllib.parse import urlparse

from app.config import settings

class ScraperEngine:
    def __init__(self):
        self.headers = {
            "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/126.0.0.0 Safari/537.36",
            "Accept-Language": "pl-PL,pl;q=0.9,en-US;q=0.8,en;q=0.7"
        }

    async def execute(self, url: str) -> Dict[str, Any]:
        """Główna metoda pobierająca i analizująca treść z dowolnego URL."""
        parsed_url = urlparse(url)
        domain = parsed_url.netloc.lower()

        # Pobranie czystego HTML
        html_content = await self._fetch_html(url)
        if not html_content:
            return {
                "scraped_success": False,
                "error_details": "Nie udało się połączyć ze stroną ogłoszenia. Oferta może być nieaktywna."
            }

        # Dedykowane dedykowane parsery dla głównych portali
        if "olx.pl" in domain:
            return self._parse_olx(html_content, url)
        elif "otomoto.pl" in domain:
            return self._parse_otomoto(html_content, url)
        else:
            # Uniwersalny Parser AI dla DOWOLNEJ innej witryny WWW
            return self._parse_universal_ai(html_content, url)

    async def _fetch_html(self, url: str) -> Optional[str]:
        """Pobiera zawartość HTML (z ew. wykorzystaniem ScraperAPI jeśli jest skonfigurowany)."""
        try:
            if settings.SCRAPERAPI_KEY:
                api_url = f"http://api.scraperapi.com?api_key={settings.SCRAPERAPI_KEY}&url={url}"
                async with httpx.AsyncClient(timeout=15.0) as client:
                    resp = await client.get(api_url)
                    if resp.status_code == 200:
                        return resp.text

            # Bezpośrednie pobranie jeśli brak klucza ScraperAPI
            async with httpx.AsyncClient(timeout=10.0, headers=self.headers, follow_redirects=True) as client:
                resp = await client.get(url)
                if resp.status_code == 200:
                    return resp.text
        except Exception as e:
            print(f"[ScraperEngine Error] {e}")
        return None

    def _parse_olx(self, html: str, url: str) -> Dict[str, Any]:
        """Dedykowany parser OLX."""
        soup = BeautifulSoup(html, "html.parser")
        title_elem = soup.find("h1", {"data-cy": "ad_title"}) or soup.find("h3")
        title = title_elem.get_text(strip=True) if title_elem else "Ogłoszenie OLX"

        price_elem = soup.find("h3", {"data-cy": "ad_price"})
        price_raw = price_elem.get_text(strip=True) if price_elem else ""

        desc_elem = soup.find("div", {"data-cy": "ad_description"})
        desc = desc_elem.get_text(separator="\n", strip=True) if desc_elem else ""

        return {
            "source_url": url,
            "title_raw": title,
            "price_raw": price_raw,
            "description_raw": desc,
            "parser_type": "DEDICATED_OLX",
            "scraped_success": True
        }

    def _parse_otomoto(self, html: str, url: str) -> Dict[str, Any]:
        """Dedykowany parser Otomoto."""
        soup = BeautifulSoup(html, "html.parser")
        title_elem = soup.find("h1") or soup.find("title")
        title = title_elem.get_text(strip=True) if title_elem else "Ogłoszenie Otomoto"

        return {
            "source_url": url,
            "title_raw": title,
            "price_raw": "",
            "description_raw": "",
            "parser_type": "DEDICATED_OTOMOTO",
            "scraped_success": True
        }

    def _parse_universal_ai(self, html: str, url: str) -> Dict[str, Any]:
        """Uniwersalny Parser AI — obsługuje dowolną stronę WWW na świecie."""
        soup = BeautifulSoup(html, "html.parser")

        # 1. Próba wyciągnięcia metadanych OpenGraph (og:title, og:description, og:price)
        og_title = soup.find("meta", property="og:title")
        og_desc = soup.find("meta", property="og:description")
        og_price = soup.find("meta", property="product:price:amount") or soup.find("meta", property="og:price:amount")

        title = og_title["content"] if og_title and "content" in og_title.attrs else ""
        description = og_desc["content"] if og_desc and "content" in og_desc.attrs else ""
        price = og_price["content"] if og_price and "content" in og_price.attrs else ""

        # 2. Jeśli brak OpenGraph, pobieramy standardowe tagi HTML (<title>, <h1>, <article>)
        if not title:
            h1 = soup.find("h1")
            title = h1.get_text(strip=True) if h1 else (soup.title.string if soup.title else "Ogłoszenie bez tytułu")

        if not description:
            # Czyszczenie ze zbędnych elementów (script, style, nav, footer)
            for tag in soup(["script", "style", "nav", "footer", "header"]):
                tag.decompose()
            
            main_content = soup.find("main") or soup.find("article") or soup.body
            if main_content:
                paragraphs = [p.get_text(strip=True) for p in main_content.find_all(["p", "li"])]
                description = " ".join(paragraphs[:15])  # Pierwsze 15 akapitów/punktów

        # 3. Szukanie wzorców cenowych w tekście (np. "125 000 PLN" lub "12 500 zł")
        if not price:
            price_match = re.search(r'(\d[\d\s\xa0]{2,}\s*(?:zł|PLN|EUR|\$))', html, re.IGNORECASE)
            if price_match:
                price = price_match.group(1).strip()

        return {
            "source_url": url,
            "title_raw": title,
            "price_raw": price,
            "description_raw": description[:3000],  # Ograniczenie wielkości opisu
            "parser_type": "UNIVERSAL_AI_FALLBACK",
            "scraped_success": True
        }