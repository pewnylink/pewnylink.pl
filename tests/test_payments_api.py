...
async def test_create_checkout_session_success(
    ...
    """Weryfikuje tworzenie sesji checkout Stripe dla istniejącego raportu bez naruszania bazy dev."""
    # 1. Tworzenie raportu testowego w IZOLOWANEJ bazie RAM
    ...
    report = await ReportRepository.create_report(db_session, report_doc)
    ...
    with patch("stripe.checkout.Session.create", return_value=mock_session):
        response = await async_client.post(f"/api/v1/payments/checkout/create-session/{report_id}")
...
async def test_stripe_webhook_unlocks_report(
    ...
    """Weryfikuje, czy zdarzenie płatności z Stripe webhooka odblokowuje raport w bazie RAM."""
    # 1. Tworzenie zablokowanego raportu w IZOLOWANEJ bazie RAM
    report_doc = await generate_audit_report(
    ...
    # 4. Odświeżenie stanu i sprawdzenie, czy raport został odblokowany w bazie RAM
    updated_report = await ReportRepository.get_by_report_id(db_session, report_id)
